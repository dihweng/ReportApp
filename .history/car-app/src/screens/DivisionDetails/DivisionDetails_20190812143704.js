  ...
  readLater = async(id, index) =>{
    ...
    try {
      ...
      if(res.status >= 200 && res.status < 300) {
        ...
        this.showNotification('success', 'Success', 'Report Added Successfully');

      }
      else {
        this.showNotification('error', 'Message', 'Failed to Add Report');
      }
    }
    catch(error) {
      this.showNotification('error', 'Message', error.toString());
  ...
  addFavorite = async(id, index) =>{
    ...
    try {
      ...
      if (res.status >= 200 && res.status < 300) {    
        ...
        this.showNotification('error', 'Message', 'Failed to Add Favorite');
      }
