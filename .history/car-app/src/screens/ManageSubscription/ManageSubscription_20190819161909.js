...
export default class ManageSubscription extends Component {
  ...
  async componentDidMount(){
    ...
    HTMLFormControlsCollection.log({userDetails})
    const id = userDetails.data.id,
      name = userDetails.data.name,
     // total_active_devices = userDetails.data.total_active_devices,
  ...
  console.log({planType})
    ...
    if(plantype == 'individual') {
      let progresslength = (loggedInDevices/3)*100;
       return this.setState({progresslength})
      //return progresslength;
    }
    else {
      let progresslength = (loggedInDevices/100)*100;
      return this.setState({progresslength});
  ...
  render () {
    const { showLoading, progresslength} = this.state;
