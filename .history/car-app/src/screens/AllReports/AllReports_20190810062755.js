...
//With the Params id, content and except
  ...
  addFavorite = async(id, index) =>{
    ...
    .then((res) => {
      if (res.status >=300 && res.status <= 500) {
        ...
        return this.showNotification(message,  'Message');
      }
      else {
        this.showNotification('Report Added To Favorite', 'Success');
        let targetPost = data[index];
        targetPost.is_favorite = !targetPost.is_favorite;
  ...
  handleAddFavorite = async(id, index) =>{
    try {
      await this.addFavorite(id, index)
    }
    catch(error) {
    this.hideLoadingDialogue();
    return this.showNotification(error.toString());
  ...
  readLater = async(id, index) =>{
    ...
    if(res.status >= 200 && res.status < 300) {
        this.showNotification('Report Added to Read Later', 'Success');
        let targetPost = data[index];
        targetPost.is_future_saved = !targetPost.is_future_saved;
        return this.setState({ data });
     }
     else {
      this.showNotification('Failed to Add Report', 'Message');
    }
  } 
  ...
  deleteFavorite=async(id, index)=> {
    ...
    try {
      ...
      let res = await response;
      if(res.status >= 200 && res.status < 300) {
        await this.showNotification('Successfully Removed Favorite', 'Success');   
        let targetPost = await  data[index];
        targetPost.is_favorite = await !targetPost.is_favorite;
        return  await this.setState({ data });
  ...
  deleteReadLater=async(id, index)=> {
    ...
    try {
      ...
      if(res.status >= 200 && res.status < 300) {
        await this.showNotification('Successfully Removed Report from Read Later', 'Success'); 
        let targetPost = await data[index];
        targetPost.is_favorite = await !targetPost.is_favorite;
        return await this.setState({ data });
      }
